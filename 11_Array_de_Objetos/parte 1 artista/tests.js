...
describe("Trabajar con Info artista",()=>{
    ...
    it("Filtrar artistas por edad - Ej 2 - artistasPorEdad",()=>{
      const edad = 30;
  ...
  it("Devuelva un array con les artistas que tiene más de esa cantidad de discos, ordenados de mayor a menor según cantidad de discos - Ej 5 - artistasConMasDiscosQue",()=>{
    const cantidadDeDiscos = 2;

      expect(artistasConMasDiscosQue(artistaTest, cantidadDeDiscos)).to.be.eql([artistaTest[0],artistaTest[2]])
  })
  it("Devuelva el objeto artista que más recaudó en su último recital - Ej 7 - artistaConMayorRecaudacion",()=> {
    
      expect(artistaConMayorRecaudacion(artistaTest)).to.be.eql(artistaTest[0])
  })
  it("Devuelva true un array con los artistas que tengan publicado al menos un disco en dicho año - Ej 8 - artistasConDiscoEnAnio",()=> {
    const anio = 1993;
    expect(artistasConDiscoEnAnio(artistaTest,anio)).to.be.eql([artistaTest[0],artistaTest[2]])
})
